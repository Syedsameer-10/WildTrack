...
export default function App() {
  ...
  const pageTitle = route === "map" ? "Spatial map" : route === "temporal" ? "Temporal history" : route === "events" ? "NoSQL events" : route === "alerts" ? "Active alerts" : route === "login" ? "Secure access" : "Wildlife intelligence";
  const routeParams = new URLSearchParams(window.location.hash.split("?")[1] ?? "");

  return <main>
    <nav className="topbar" aria-label="Primary navigation"><a className="brand" href="#/"><span className="brand-mark">W</span><span>WildTrack</span></a><div className="section-nav"><a href="#/map">Map</a><a href="#/temporal">Temporal</a><a href="#/events">Events</a><a href="#/alerts">Alerts</a><a className="login-nav" href="#/login">Sign in</a></div><span className="phase-badge">{pageTitle}</span></nav>
